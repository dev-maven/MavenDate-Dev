...
namespace MavenDate.API.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        public async Task<Photo> GetMainPhtoForUser(int userId)
        {
        ...
        public async Task<PagedList<Message>> GetMessagesForUser(MessageParams messageParams)
        {
            ...
            {
                case "Inbox": 
                    messages = messages.Where(u => u.RecipentId == messageParams.UserId && u.RecipentDeleted == false);
                    break;
                case "Outbox":
                    messages = messages.Where(u => u.SenderId == messageParams.UserId && u.SenderDeleted == false);
                    break;
                default:
                    messages = messages.Where(u => u.RecipentId == messageParams.UserId && u.IsRead == false && u.RecipentDeleted == false);
                    break;
            }
        ...
        public async Task<IEnumerable<Message>> GetMessageThread(int userId, int recipentId)
        {
            var messages = await _context.Messages
                .Include(u => u.Sender).ThenInclude(p => p.Photos)
                .Include(u => u.Recipent).ThenInclude(p => p.Photos)
                .Where(m => m.RecipentId == userId && m.RecipentDeleted == false
                 && m.SenderId == recipentId 
                 || m.RecipentId == recipentId 
